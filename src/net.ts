...
export class Net{
  ...
  constructor(shapeHeight: number, levelMapArr: number[][], levelMapImage: Image){
  ...
  clickAt(sketch: p5){
    ...
    if(mx < 0 || my < 0 || mx > sketch.width || my > sketch.height) return
    let lowestDist
    let closestCell
  ...
  spacesOfType(type: number){
    ...
    for(let   j = 0; j<this.shapes.length; j++){
      for(let i = 0; i<this.shapes[j].length; i++){
        if(this.shapes[j][i].type === type) matchingSpaces.push(this.shapes[j][i])
      }
    }
