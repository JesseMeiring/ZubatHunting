    ...
    buildNet(levelMapArr) {
        ...
        for (let i = 0; i < levelMapArr[0].length; i++) {
            ...
            for (let j = 0; j < levelMapArr.length; j++) {
                ...
                let cell = new shape_1.Shape(this.shapeHeight, thisX, posY, rotation, cellType);
...
class net{
    ...
    for(let   j = 0; j<this.shapes.length; j++){
      for(let i = 0; i<this.shapes[j].length; i++){
        ...
        for(let tj = -1; tj < 2; tj++){
          if(tj == 0) {
            for(let ti of sameRowIndecies){
              if(this.checkShapeExists(i + ti, j + tj)) indexShape.neighbors.push(this.shapes[j + tj][i + ti])
            }
          } else {
            if(j % 2 == 0) {
              for(let ti of onRowIndecies){
  ...
  clickAt(){
    ...
    for(let r of this.shapes) {
      for(let cell of r) {
        let distance = dist(mx, my, cell.center.x, cell.center.y)
        if(lowestDist == null || lowestDist > distance) {
          lowestDist = distance
          closestCell = cell
        }
      }
    ...
    //if the closest distance is within half-ish a shapeheight, we can assume it's on that shape and return it
    if(lowestDist < this.shapeHeight / 1.6) return closestCell
    //otherwise, we probably didn't click on a cell
    return null
  }
  ...
  spacesOfType(t){
    ...
    for(let   j = 0; j<this.shapes.length; j++){
      for(let i = 0; i<this.shapes[j].length; i++){
        if(this.shapes[j][i].type == t) matchingSpaces.push(this.shapes[j][i])
      }
    }
    return matchingSpaces
  }
...
//   }
  ...
  withinTwoMovement(t1, t2){
    for(let n1 of t1.neighbors.filter((n) => n.type != 5)) {
      if(n1 == t2) return true
      for(let n2 of t2.neighbors.filter((n) => n.type != 5)) {
