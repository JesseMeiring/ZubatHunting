...
export class Shape{
    ...
    constructor(shapeHeight: number, 
      xCenter: number, 
      yCenter: number, 
      angle: number, 
      type: number
      ...
      for(let pI = 0; pI<this.pointCount; pI++){
        this.points.push(new Vector(
          this.r * Math.cos(this.angle + (pI/this.pointCount) * 2 * Math.PI) + this.center.x,
          this.r * Math.sin(this.angle + (pI/this.pointCount) * 2 * Math.PI) + this.center.y
        ))
      }    
      ...
      for(let pI = 0; pI<this.pointCount; pI++){
        this.highlightPoints.push(new Vector(
          (this.r - 2) * Math.cos(this.angle + (pI/this.pointCount) * 2 * Math.PI) + this.center.x,
          (this.r - 2) * Math.sin(this.angle + (pI/this.pointCount) * 2 * Math.PI) + this.center.y
        ))
      }
    ...
    drwshp(sketch: Graphics, displayData){
      ...
      if(displayData.showColorTypes) {
        ...
        if(this.type == 3) sketch.fill(250, 150, 0, displayData.tileColorOpacity);    // const PLAYER_SPAWN = 3
        if(this.type == 4) sketch.fill(250, 0, 250, displayData.tileColorOpacity);    // const PREY_SPAWN = 4
      }
      ...
      if(visibleSpriteCount == 1 && !displayData.showLabels) {
        ...
        for(let pI = 0; pI<visibleSpriteCount; pI++){
          ...
          spriteCenters.push(new Vector(
            this.r * spriteOffset * Math.cos(this.angle - Math.PI/3 + (pI/visibleSpriteCount) * 2 * Math.PI) / 2 + this.center.x,
            this.r * spriteOffset * Math.sin(this.angle - Math.PI/3 + (pI/visibleSpriteCount) * 2 * Math.PI) / 2 + this.center.y
          ))
        }
      }
